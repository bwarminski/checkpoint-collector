...
**Scope Note:**
- `load/lib/load/fixture_verifier.rb` stays as-is in this refactor. It still contains workload-owned todo behavior, but moving it is outside this seam.
...
Also bump `initial_state[:schema_version]` from `1` to `2` so the persisted run format reflects the breaking `invariant_samples` shape change.
...
BUNDLE_GEMFILE=collector/Gemfile bundle exec ruby load/test/runner_test.rb --name test_invariant_check_with_nil_min_and_max_never_breaches
BUNDLE_GEMFILE=collector/Gemfile bundle exec ruby load/test/runner_test.rb --name test_invariant_check_actual_equal_to_min_does_not_breach
BUNDLE_GEMFILE=collector/Gemfile bundle exec ruby load/test/runner_test.rb --name test_invariant_sample_with_empty_checks_is_healthy
...
BUNDLE_GEMFILE=collector/Gemfile bundle exec ruby load/test/runner_test.rb --name test_runner_initial_state_uses_schema_version_2
...
Mechanical conversion sweep in `load/test/runner_test.rb`:
- the three samples in `test_runner_aborts_after_three_consecutive_invariant_breaches`
- the three samples in `test_runner_records_invariant_breach_before_first_successful_request`
- the five samples in `test_runner_does_not_abort_when_breach_recovers`
- the two samples in `test_runner_persists_invariant_samples_in_run_record`
- the fallback sample in `FakeInvariantSampler#call`
- the default sample in `RecordingInvariantSampler#call`
...
Do not add an `invariant_sampler` override to `MetricsWorkload`; the continuous-mode error test depends on it inheriting the new default `nil` hook from `Load::Workload`.
...
for i in $(seq 1 50); do
  BUNDLE_GEMFILE=collector/Gemfile bundle exec ruby \
    workloads/missing_index_todos/test/invariant_sampler_test.rb \
    --name test_sampler_uses_isolated_pg_connection_and_returns_named_checks 2>&1 | tail -3
...
grep -rn "todos\|open_count\|total_count" load/lib/load/runner.rb load/lib/load/scale.rb load/lib/load/workload.rb
cat runs/<latest-run-dir>/run.json | jq .schema_version
```

Expected: the two grep commands print nothing and `jq` prints `2`.
...
- Spec coverage: Task 1 covers the `Scale` shape, env ordering, reserved-key rejection, and call-site sweep; Task 2 covers the generic invariant contract, schema-version bump, and base workload hook; Task 3 covers sampler extraction, workload-owned threshold math, and the PG failure contract; Task 4 covers runner integration and error handling; Task 5 covers validation, narrowed leak checks, and the schema-version gate.
