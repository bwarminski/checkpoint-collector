...
- Modify `adapters/rails/lib/rails_adapter/commands/reset_state.rb`: remove hardcoded `QUERY_IDS_SCRIPT`, resolve optional workload-owned Rails query-id script by convention, run its contents with `bin/rails runner`.
...
```ruby
  def test_reset_state_remote_strategy_skips_template_cache_and_runs_schema_seed_and_stats_steps
    Dir.mktmpdir do |workload_root|
      ...
      write_workload_query_ids_script(root: workload_root, workload: "missing-index-todos", body: script_body)
      query_ids_json = %({"query_ids":["111"]})
      runner = FakeCommandRunner.new(
        results: {
          ["bin/rails", "runner", script_body] => FakeResult.new(status: 0, stdout: query_ids_json, stderr: ""),
        },
      )
...
- [ ] **Step 2: Add `workload_root` to the initializer**

Change the initializer signature in `ResetState` to:

```ruby
      def initialize(app_root:, seed:, env_pairs:, workload: nil, command_runner: RailsAdapter::CommandRunner.new, template_cache: RailsAdapter::TemplateCache.new, reset_strategy: ENV.fetch("BENCH_ADAPTER_RESET_STRATEGY", "local"), workload_root: File.expand_path("../../../../../workloads", __dir__), clock: -> { Time.now.to_f })
        @app_root = app_root
        @workload = workload
        @seed = seed
        @env_pairs = env_pairs
...
- [ ] **Step 3: Replace query-id capture with script-file lookup**
...
- [ ] **Step 4: Run reset-state tests and verify progress**
...
```ruby
  def test_reset_state_reports_workload_query_id_script_failure_details
    Dir.mktmpdir do |workload_root|
      ...
      runner = FakeCommandRunner.new(
        results: {
          ["bin/rails", "runner", script_body] => FakeResult.new(status: 1, stdout: "", stderr: "query lookup failed"),
        },
      )
      command = RailsAdapter::Commands::ResetState.new(
        app_root: "/tmp/demo",
...
```ruby
  def test_reset_state_reports_when_workload_query_id_script_omits_query_ids
    Dir.mktmpdir do |workload_root|
      ...
      runner = FakeCommandRunner.new(
        results: {
          ["bin/rails", "runner", script_body] => FakeResult.new(status: 0, stdout: %({"ok":true}), stderr: ""),
        },
      )
      command = RailsAdapter::Commands::ResetState.new(
        app_root: "/tmp/demo",
...
- [ ] **Step 5: Run reset-state tests with the real workload file present**
...
- [ ] **Step 6: Commit workload script**

Run:

```bash
git status --short
git add workloads/missing_index_todos/rails/reset_state_query_ids.rb workloads/missing_index_todos/test/workload_test.rb
...
- Spec coverage: the plan moves the script out of `ResetState`, preserves Rails execution in the adapter, supports missing scripts, covers omitted `query_ids`, preserves `run.json.query_ids`, and keeps behavior for `missing-index-todos`.
